                    ...
                    {folders.map(( folder ) => (
                        <div key={folder.key}>
                            <div className="block mt-[10px] p-1 rounded-md flex items-center duration-300 hover:bg-zinc-700 hover:duration-200">
                                <input onChange = { (e) => { setFormArticleValue({ folder: e.target.value}) ;console.log('folder_key',[e.target.value])} }className="rounded-md bg-zinc-400 focus:bg-sky-900 text-sky-900 focus:ring-0" type="checkbox" key={folder.id} value={folder.key} id={folder.id}/>
