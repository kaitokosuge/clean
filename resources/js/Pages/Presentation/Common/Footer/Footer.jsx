...
function Footer({ folders ,handleFormSubmit , setFormValue }) {
    ...
    return (
        <div className="w-[82%] min-w-[50%] h-[150px] fixed bottom-0 z-10 right-0 bg-shadow flex items-center">
            ...
            <div onMouseLeave={ handleHover } className={ isHover == true ? "absolute bottom-0 right-[11%] block opacity-100 visible border border-zinc-600 bg-zinc-900 rounded-[5px] px-5 py-[10px] max-h-[400px] overflow-scroll duration-300" : "absolute bottom-0 right-[11%] opacity-0 duration-300 invisible max-h-[400px]"}>
                ...
                {folders.map(( folder ) => (
                    <>
                    <div className="block mt-1 p-1 rounded-md flex items-center duration-300 hover:bg-zinc-700 hover:duration-200">
