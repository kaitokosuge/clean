            ...
            <li className="relative">
                <a href={article.url} target="_blank" onMouseEnter={handleLinkHover} onMouseLeave={handleLinkHover}className= "flex w-full p-5 rounded-md duration-200 mt-1 hover:bg-zinc-700 hover:duration-200">
                    ...
                    <div className="ml-5 limit">
                        ...
                        <div className={isLinkHover == true ? "flex duration-300 opacity-100 visible mt-1" : "flex opacity-0 duration-500 invisible" }>
                ...
                <div onMouseLeave={handleHover} className={ isHover == true ? "opacity-100 visible w-full h-[400px] relative top-[-50px] bg-white rounded-xl px-10 py-5 z-70 duration-300" : "relative top-[-50px] opacity-0 duration-500 invisible w-0 h-0" }>
                ...
                <div onMouseLeave={handleToFolderHover} className={ isToFolderHover == true ? "opacity-100 visible w-[50%] relative top-[-50px] h-[400px] bg-zinc-900 rounded-xl p-5 z-100 duration-300 overflow-scroll" : "relative top-[-50px] opacity-0 duration-500 invisible w-0 h-0" }>
                    <p className={ isToFolderHover == true ? 'text-gray-500 font-bold opacity-100 visible limit': 'text-black font-bold opacity-0 duration-300 invisible h-0'}>{article.title}</p>
                    {folders.map(( folder ) => (
                        <div key={folder.id}>
